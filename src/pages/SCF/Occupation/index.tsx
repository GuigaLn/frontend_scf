...
const Occupation: React.FC = () => {
  ...
  const [value, setValue] = useState<valueOccupation>({id: 0, description: "Error", idoccupation: 0});
  const [description, setDescription] = useState('');
  const [data, setData] = useState<any>(data2);
  const columns: any = [
    {
      name: "ID",
      selector: (row: any) => row.idoccupation,
      sortable: true
    },
    {
      name: "Descrição",
  ...
  let promiseAdd = () => {
    const reseolveApi = new Promise((resolve, reject) => {
      try {
        api.post('/occupation', {description: description}).then(response => {
          setData([...data, {id: response.data.rows[0].id, description, idoccupation: response.data.rows[0].id}]);
          setTimeout(resolve);
  ...
  let promiseEdit = () => {
    const reseolveApi = new Promise((resolve, reject) => {
      try {
        api.put('/occupation', {description: value.description, id: value.id}).then(response => {
          reloadData();
          setTimeout(resolve);
          setOpenModal(false);
          return;
  ...
  let promiseDelete = () => {
    const reseolveApi = new Promise((resolve, reject) => {
      try {
        api.post('/occupation/delete', {id: value.id}).then(response => {
          reloadData();
          setTimeout(resolve);
          setOpenModal(false);
          return;
